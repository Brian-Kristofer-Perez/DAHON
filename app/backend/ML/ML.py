...
class MLModel:
    ...
    def __init__(self, path_to_model: str):
        ...
        self.model = torch.load(path_to_model, weights_only= False, map_location=torch.device('cpu'))
    ...
    def predict(self, imgbytes: BytesIO) -> tuple[str, str]:

        image = Image.open(imgbytes)
        results = self.model.forward(self.preprocess(image).unsqueeze(0))
